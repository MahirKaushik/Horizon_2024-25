...
const Carousel = ({ cardData }) => {
  ...
  return (
    <div className="carousel-container relative overflow-hidden flex">
      <button className="carousel-button left absolute top-1/2 -translate-y-1/2 cursor-pointer w-[88px] h-[88px] z-[10] left-[2.8%]" onClick={goLeft}>
        ...
        <div className="carousel-content flex transition-transform duration-300 ease-in-out" style={{ transform: `translateX(-${currentIndex * (100 / cardData.length)}%)` }}>
          {cardData.map(( index) => (
            <div className="carousel-item w-screen my-[150px] flex justify-center" key={index}>
              <Card /> 
            </div>
          ))}
        </div>
